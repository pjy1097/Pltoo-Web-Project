...
function togglePasswordVisibility01() {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateAllInputs() {
        return checkPw() && comparePw() && checkName() && isBirthCompleted() && checkGender() && isEmailCorrect() && checkPhoneNum();
    }
    ...
    function isEmailCorrect() {
        ...
        if (email.value === "") {
            error[6].innerHTML = "이메일 주소를 입력해주세요.";
            error[6].style.display = "block";
            return false;
        } else if (!emailPattern.test(email.value)) {
            error[6].innerHTML = "유효한 이메일 주소를 입력해주세요.";
            error[6].style.display = "block";
            return false;
        } else {
